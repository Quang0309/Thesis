...
import os 
from fairseq.data import Adj_Generator as AdjGenerator


def collate(
    samples, pad_idx, eos_idx,  adj_generator,  left_pad_source=False, left_pad_target=False,
    input_feeding=True,
):
    if len(samples) == 0:
        return {}
    ...
    src_tokens = merge('source', left_pad=False)#left_pad_source) # edit to false
    # sort by descending source length
    src_lengths = torch.LongTensor([s['source'].numel() if s['source'].numel() <=50 else 50  for s in samples]) #here
    ...
    batch = {
        ...
        'net_input': {
            ...
            'adjInverseTensor': adjInverseTensor,
            'labelInverseTensor': labelInverseTensor,
        },
        'target': target,
    }
...
class LanguagePairDataset(FairseqDataset):
    ...
    def __init__(
        ...
        if (split == 'train'):
            ...
            name = 'test.en.out'
        self.adj_generator = AdjGenerator.AdjGenerator(name, numberOfWords=50, batchSize=32)
